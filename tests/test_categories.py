...
def test_update_category(user_token):
    ...
    response = client.put(f"{API_PREFIX}/categories/{category_id}", json={"name": new_name}, headers=auth_headers(user_token))
...
def test_update_nonexistent_category(user_token):
    response = client.put(f"{API_PREFIX}/categories/999999", json={"name": unique_name("Nonexistent")}, headers=auth_headers(user_token))
